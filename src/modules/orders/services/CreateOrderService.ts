...
import Order from "../typeeorm/entities/Order";
import { OrdersRepository } from "../typeeorm/repositories/OrdersRepository";
...
class CreateOrderService {
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    ...
    if (checkInexistentsProducts.length) {
      throw new AppError(`Could not find product with ${checkInexistentsProducts[0].id}.`);
    }
